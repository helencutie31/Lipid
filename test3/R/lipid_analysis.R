...
lipid_analysis_server <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    # MS2 attach + annotate phai chay rieng tung polarity (metid can 1
    # polarity/1 lan goi), nen POS/NEG duoc giu tach o buoc 1 va 2; chi merge
    # thanh 1 mass_dataset duy nhat o CUOI buoc 2 (xem R/annotation.R).
    pipeline_state <- shiny::reactiveValues(
      work_dir = NULL,             # thu muc raw mzXML tu buoc 1 - song den het buoc 3
      mass_dataset_pos = NULL,
      mass_dataset_neg = NULL,
      pp_done = FALSE,
      pp_summary = NULL,
      
      mass_dataset = NULL,         # merged POS+NEG, ghi boi buoc 2 sau khi annotate
    ...
    annotation_server("annot", pipeline_state)
...
.lfs_parse_msdial_msp <- function(file_path) {
  ...
  records <- purrr::map(seq_along(block_start), function(i) {
    ...
    spec <- peak_lines %>%
      stringr::str_split("[\\s\\t]+") %>%
      purrr::map(~ suppressWarnings(as.numeric(.x[1:2]))) %>%
      do.call(rbind, .) %>%
      as.data.frame()
